...
// Response type middleware to ensure all API responses are properly marked as JSON
...
if (process.env.NODE_ENV === "production") {
  app.use(
    express.static(path.join(__dirname, "dist"), {
      setHeaders: (res, path) => {
        // Explicitly set MIME types for image files
...
app.delete("/api/books/:id", authenticateToken, async (req, res) => {
  try {
    ...
    await db.deleteBook(id, req.user.id)
...
app.post("/api/books/:id/toggle", authenticateToken, async (req, res) => {
  try {
    ...
    await db.toggleBookStatus(id, req.user.id)

    res.json({
      message: "Book status toggled successfully",
      id: Number.parseInt(id),
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log("──────────────────────────────────────")
      console.log(`📍 Running on DATABASE_URL`)
